...
Each vertex of the cube, \( \vb{v}_i \in \R^3\), is stored in a large array \( \vb{V} \in \R^{8 \times 3} \), as shown in~\cref{eq:cube_vertices}.
The vertices for all polyhedron shape models are typically stored in a body fixed reference system centered at the origin of the shape and aligned with the priniciple axes of the body.
\begin{align}~\label{eq:cube_vertices}
    \vb{V} = \begin{bmatrix}
        \begin{array}{ccc}
        -0.5 & -0.5 & -0.5 \\
        -0.5 & -0.5 & 0.5  \\
