...
\begin{align}\label{eq:relative_lagrangian}
    L = T-V =& \frac{1}{2} \parenth{m_1 + m_2} \norm{\gls{sym:rv}}^2 + \frac{1}{2} \tr{\hat{\gls{sym:rW}} J_{d_R} \hat{\glsentrytext{sym:rW}}^T} \nonumber \\
             &+m_1 U(\gls{sym:rx} + \gls{sym:rR} \vc{\rho}_1) + m_2 U(\gls{sym:rx} + \gls{sym:rR} \vc{\rho}_2) .
\end{align}
...
\begin{align}
    \delta \gls{sym:rx} &= \left. \diff{}{\epsilon} \right|_{\epsilon=0} \gls{sym:Ra}[^T] \gls{sym:ix}^\epsilon ,\nonumber \\
                        &= \gls{sym:Ra}[^T] \delta \gls{sym:ix} , \nonumber\\
                        &= \gls{sym:rxvar}.\label{eq:relative_position_variation}
\end{align}

The variation of the relative attitude given in~\cref{eq:relative_attitude} is expressed as
\begin{align}
    \delta \gls{sym:rR} &= \left. \diff{}{\epsilon} \right|_{\epsilon=0}  \gls{sym:Ra}[^T] \gls{sym:iR}^\epsilon = \gls{sym:Ra}[^T] \delta \gls{sym:iR}, \nonumber\\
                        &= \gls{sym:Ra}[^T] \gls{sym:iR} \eta,
\end{align}
...
The variation of the spacecraft attitude \( \gls{sym:ietavar} \in \so \) is related to the relative variation \( \gls{sym:retavar} \in \so \) by
\begin{align*}
