...
\end{figure}
\section{Autonomous Guidance}\label{sec:explore_asteroid}
...
\end{align}
where the external force is defined by the polyhedron potential model given in
\begin{align}
    J_{c_i}(\rpos) = \int_{\theta_0}^{\theta_f} u_f(\theta)^T R u_f(\theta) d\theta.
\end{align}
This term penalizes vertices that would require a large control input in order to reach.

% TODO Cost function to decide on which piont to measure
Want to enable autonomous exploration.

Need to determine next state to move and where to measure

Each vertex of shape model has an associated uncertainty.

Goal is to minimize the uncertainty

Too expensive to use a global optimization scheme and solve the optimal control problem

Cost function is weighted sum of: distance, uncertainty, control cost

Once we have a state we can manuever spacecraft there using \( \SE \) control and point at asteroid


\subsection{Landing Area Selection and Refinement}

Select a specific landing site and refine the area

Several metrics can be used to determine and appropriate landing site

Surface slope (gravity vector from scheeres paper)

Uncertainty

Estimated science value. Real mission would require much more extensive mapping but this approach demonstrates that autonmous mapping is possible

Use mesh refinement to subdivide a specific area and then take more measurements

\section{Shape Reconstruction Examples}
\subsection{Kinematics based reconstruction}
