...
In this section, we demonstrate the use of the results presented in~\cref{sec:se3_control,sec:shape_reconstruction} with several numerical examples.
...
\Cref{sec:52760_results} incorporates the full dynamic model on \( \SE \) of a dumbbell spacecraft given in~\cref{sec:inertial_dumbbell_eoms}, the polyhedron potential model from~\cref{sec:polyhedron_potential}, and the mesh update and guidance scheme presented in~\cref{sec:radius_update}.
Finally,~\cref{sec:castalia_results} includes a full dynamic simulation and the landing area refinement approach presented in~\cref{sec:landing_refinement}.

\subsection{Incremental Mesh Update Examples}\label{sec:kinematic_exploration}

The incremental mesh update approach from~\cref{sec:radius_update} is used to iteratively update an initial shape estimate with range measurements of the surface.
The results presented in this section are based on a simplified model to demonstrate the application and the computational efficiency of the approach.
The simulations use a kinematics only model that neglects the effect of gravity and the motion of the asteroid. 
In addition, we do not implement the nonlinear geometric controllers of~\cref{sec:se3_control} but instead assume the spacecraft and sensor can be positioned arbitrarily.
...
The simulations begin with an triaxial ellipsoid triangular mesh that is sized to match the semi-major axes of the asteroids.
The number of vertices is chosen to be approximatly equal to that of the radar shape model.
...
\begin{figure}[htbp]
    \centering
    \subcaptionbox{Initial Shape Estimate\label{fig:52760_partial_0}}{\includegraphics[height=0.5\textheight,width=0.5\textwidth,keepaspectratio]{figures/computational_geometry/mesh_update/52760/partial_0}}~
    \subcaptionbox{\SI{25}{\percent} of measurements added\label{fig:52760_partial_25}}{\includegraphics[width=0.5\textwidth]{figures/computational_geometry/mesh_update/52760/partial_258}}

    \subcaptionbox{\SI{50}{\percent} of measurments added\label{fig:52760_partial_50}}{\includegraphics[width=0.5\textwidth]{figures/computational_geometry/mesh_update/52760/partial_517.jpg}}~
    \subcaptionbox{\SI{75}{\percent} of measurements added\label{fig:52760_partial_75}}{\includegraphics[width=0.5\textwidth]{figures/computational_geometry/mesh_update/52760/partial_775.jpg}}

    \subcaptionbox{\SI{100}{\percent} of measurements added\label{fig:52760_partial_100}}{\includegraphics[width=0.5\textwidth]{figures/computational_geometry/mesh_update/52760/partial_1033.jpg}}~
...
\begin{figure}[htbp]
    \centering
    \input{figures/computational_geometry/mesh_update/golevka/volume.pgf}
\end{figure}
%TODO Castalia
...
\subsection{Asteroid 52760}\label{sec:52760_results}
% TODO Show examples of shape reconstruction (Castalia, using C++ code for speed)
Show exploration
\subsection{4769 Castalia}\label{sec:castalia_results}
