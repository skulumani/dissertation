...
\begin{align}
    A(R, R_d) &= \frac{1}{2} G \tr{I - R_d^T R}, \label{eq:psi} \\
    e_R &= \frac{1}{2} G \parenth{R_d^T R - R^T R_d^\vee}, \label{eq:er}\\
    e_\Omega &= \Omega - R^T R_d \Omega_d \label{eq:ew}.
\end{align}
\end{subequations}
Then the following properties hold:
\begin{enumerate}
    \item \label{item:prop_psi_psd} \( A \) is locally positive definite about \( R = R_d \) on \( \SO \).
    \item \label{item:prop_er} The variation of \( A \) with respect to a variation of \( \delta R = R \hat{\eta} \) for \( \eta \in \R^3 \) is given by
...
Talk about the constrained attitude control
