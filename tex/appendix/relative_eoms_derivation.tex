...
\begin{align}
    ...
    \tr{ - \hat{\Omega}_r \gls{sym:Jdr} \gls{sym:rattvar} \hat{\Omega}_r  } &= \tr{ -\gls{sym:rattvar}  \hat{\Omega}_r  \hat{\Omega}_r \gls{sym:Jdr} } , \\
    \tr{-\hat{\Omega}_r \gls{sym:rattvar} \gls{sym:Jdr}  \hat{\Omega}_r^T} &= \tr{ \gls{sym:rattvar} \gls{sym:Jdr}  \hat{\Omega}_r \hat{\Omega}_r} , \\
    \tr{ \hat{\Omega}_r\gls{sym:Jdr} \hat{\Omega}_r\gls{sym:rattvar}} &= \tr{\gls{sym:rattvar} \hat{\Omega}_r\gls{sym:Jdr} \hat{\Omega}_r},
\end{align}
\end{subequations}     
where we made use of~\cref{eq:trABC,eq:hatxT}.
Similarly, we can perform a similar operation for~\cref{eq:relative_kinetic_energy_B_1} 
...
Substituting~\cref{eq:relative_kinetic_energy_A_simplification} into~\cref{eq:relative_kinetic_energy_A_1} and~\cref{eq:relative_kinetic_energy_B_simplification} into~\cref{eq:relative_kinetic_energy_B_1} gives:
\begin{align}
    A &= \frac{1}{2} \tr{-\gls{sym:rattvar} \hat{\Omega}_r\parenth{\gls{sym:Jdr} \hat{\Omega}_r +  \hat{\Omega}_r \gls{sym:Jdr}} + \gls{sym:rattvar} \parenth{ \gls{sym:Jdr} \hat{\Omega}_r +  \hat{\Omega}_r\gls{sym:Jdr}} \hat{\Omega}_r }, \label{eq:relative_kinetic_energy_A_2}\\
...
\end{align}
Using~\cref{eq:moi_relative_hat_prop,eq:hatxy} leads to the following simplifications of~\cref{eq:relative_kinetic_energy_A_2,eq:relative_kinetic_energy_B_2} as follows:
\begin{subequations}\label{eq:relative_kinetic_energy_A_and_B_final}
\begin{align}
    A &= \frac{1}{2} \tr{-\gls{sym:rattvar}\parenth{ \hat{\Omega}_r\parenth{\gls{sym:Jr} \hat{\Omega}_r}}^\wedge} , \\
    B &= \frac{1}{2} \tr{\gls{sym:rattvar}\parenth{ \hat{\Omega}_A\parenth{\gls{sym:Jr}  \hat{\Omega}_r}}^\wedge} .
\end{align}
\end{subequations}     
Finally, substituting~\cref{eq:relative_kinetic_energy_A_and_B_final} into~\cref{eq:relative_kinetic_energy_b_2} and then into~\cref{eq:relative_kinetic_energy_variation_1} along with using~\cref{eq:moi_relative_hat_prop} gives the final variation of the relative kinetic energy as
\begin{align}\label{eq:relative_kinetic_energy_variation}
    \delta T =& (m_1 + m_2) \gls{sym:rvel}[^T] \parenth{\delta \dot{x}_r +  \hat{\Omega}_A \delta \gls{sym:rpos}} + \frac{1}{2} \tr{ -\dot{\hat{\eta}}\parenth{\gls{sym:Jr}  \hat{\Omega}_r }} \\
             &+ \frac{1}{2} \tr{-\gls{sym:rattvar}\parenth{ \hat{\Omega}_r\parenth{\gls{sym:Jr} \hat{\Omega}_r}}^\wedge} +
